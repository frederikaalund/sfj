...
#define LIGHT_STRUCT_SIZE 6
		...
		float falloff = 1.0;//1.0 / (constant_attenuation + light_distance * (linear_attenuation + cubic_attenuation * light_distance));

		// Diffuse
		vec3 light_contribution = albedo * light_color * max(dot(wc_normal, light_direction), 0.0);

		vec3 view_direction = normalize(wc_position - wc_camera_eye_position);

		// Specular
		light_contribution += 0.2 * max(pow(dot(wc_normal, -normalize(vertex.wc_camera_ray_direction)), 50.0), 0.0);
...
void main()
{
	...
	vec3 wc_position = wc_camera_eye_position + vertex.wc_camera_ray_direction * -ec_position_z / z_far;
	vec3 wc_normal = texture(wc_normals, tc_window).xyz;
	vec3 wc_view_direction = normalize(vertex.wc_camera_ray_direction);
	...
	// Shadow Mapping
	color.rgb *= 
		 //calculate_shadow_coefficient(shadow_map_0, light_projection_matrix_0, light_view_projection_matrix_0, light_radius_0, light_direction_0, wc_position, wc_normal, tc_window, uniform_distribution_random)
