...
float calculate_shadow_coefficient( in vec3 wc_position )
{
	...
	*/
	...
	return shadow_coefficient / (11.0 * 11.0);
/*
...
#define LIGHT_STRUCT_SIZE 9
		...
		// Diffuse
		vec3 light_contribution = albedo * light_color * max(dot(bent_normal, light_direction), 0.0);
...
void main()
{
	...
	const float b = 1.0;
	const float c = 1.0;
	ambient_occlusion = pow(b * (ambient_occlusion + a), c);
	...
	// Overrides
	//color.rgb *= 1.0e-32;
	//color.rgb += vec3(-eye_z(tc_window) / z_far);
	//color.rgb += vec3(ambient_occlusion);
	//color.g += texture(ambient_occlusion_texture, tc_window).x;
	//color.rgb += texture(random_texture, tc_window).rgb;
	//color.rgb += bent_normal;
	//color.rgb += wc_normal;
	//color.rgb += albedo;
	//color.rgb += 0.5880 * calculate_direct_light2(wc_position, wc_normal, albedo, bent_normal, specular_exponent).rgb;
